...
from src.sensors import MySensors       # type: ignore
...
from src.eventhandler import MyEventHandler     # type: ignore
...
def main():
    ...
    while True:
        ...
        if water == 1:
            myDB.write_db("Water", ["location", "Basement"], "water", 1)
        ...
        if database_counter == 43:  ## 43 =ish 5 minutes
            if humid != 0:
                myDB.write_db("Temperature", ["location", "Kitchen"], "temperature", temp)
                myDB.write_db("Humidity", ["location", "Kitchen"], "humidity", humid)

            myDB.write_db("CO", ["location", "Kitchen"], "co", co_val)
            myDB.write_db("LPG", ["location", "Kitchen"], "lpg", lpg_val)
            myDB.write_db("Smoke", ["location", "Kitchen"], "smoke", smoke_val)
